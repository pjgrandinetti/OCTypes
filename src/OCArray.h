...
/**
 * @typedef OCMutableArrayRef
 * A reference to a mutable array object.
 */
typedef struct __OCArray * OCMutableArrayRef;

/**
 * @brief Callback function to retain a value in the array.
...
/**
 * @brief Callback function to release a value from the array.
 * @param value The value to be released.
 */
typedef void		(*OCArrayReleaseCallBack)(const void *value);

/**
 * @brief Callback function to create a string description of a value in the array.
...
/**
 * @brief Callback function to compare two values in the array for equality.
 ...
 * @brief A structure defining callback functions for managing values within an OCArray.
 *
 * These callbacks allow OCArray to manage the lifecycle and comparison of custom data types.
 * @var OCArrayCallBacks::version
 * The version of the callbacks structure. Should be 0.
 * @var OCArrayCallBacks::retain
 * A callback function to retain a value. Can be NULL if values do not need explicit retaining.
 * @var OCArrayCallBacks::release
 * A callback function to release a value. Can be NULL if values do not need explicit releasing.
 * @var OCArrayCallBacks::copyDescription
 * A callback function to provide a string description of a value. Can be NULL.
 * @var OCArrayCallBacks::equal
 * A callback function to compare two values for equality. Must not be NULL if OCEqual is to be used on arrays with these callbacks.
 */
typedef struct {
    int64_t     version;
...
/**
 * @brief A predefined set of callbacks for arrays containing OCType objects.
 * These callbacks use OCRetain, OCRelease, OCCopyDescription, and OCEqual.
 */
extern const OCArrayCallBacks kOCTypeArrayCallBacks;

/**
 * @brief Returns the unique type identifier for OCArray objects.
 * @return The OCTypeID associated with OCArray.
 */
OCTypeID OCArrayGetTypeID(void);

/**
 * @brief Returns the number of values currently stored in the array.
...
/**
 * @brief Creates a new, immutable array containing the provided values.
 * @details The values are retained according to the provided callbacks.
...
/**
 * @brief Creates a new, immutable array that is a copy of an existing array.
 * @details The values in the new array are retained as if they were added to the original.
...
/**
 * @brief Creates a new, mutable array with a specified initial capacity.
...
/**
 * @brief Creates a new, mutable array that is a copy of an existing array (either mutable or immutable).
 ...
 * @details The value is retained by the array using the retain callback, if one was provided during creation.
...
/**
 * @brief Appends a range of values from one array to the end of a mutable array.
...
/**
 * @brief Finds the first index at which a given value appears in the array.
 * @details Equality is determined using the `equal` callback provided during array creation.
...
/**
 * @brief Checks whether the array contains a given value.
 * @details Equality is determined using the `equal` callback provided during array creation.
 * @param theArray The array to search. Must not be NULL.
...
/**
 * @brief Removes the value at a specific index from a mutable array.
 * @details The value being removed is released by the array using the release callback, if one was provided.
...
/**
 * @brief Inserts a value at a specific index in a mutable array.
 * @details The value is retained by the array. Existing values at or after the index are shifted.
...
/**
 * @brief Sorts the values in a mutable array over a given range using a provided comparison function.
...
/**
 * @brief Performs a binary search for a value within a sorted range of an array.
 * @details The array or the specified range within it must already be sorted according to the `comparator`.
