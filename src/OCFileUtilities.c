...
bool
OCCreateDirectory(const char *path,
                  bool        recursive,
                  OCStringRef *err)
{
    if (!recursive) {
...
bool
OCRenameItem(const char    *oldPath,
             const char    *newPath,
             OCStringRef   *err)
{
...
bool
OCStringWriteToFile(OCStringRef str, const char *path, OCStringRef *err)
{
