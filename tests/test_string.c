...
bool stringTest3(void) {
    ...
    if (OCStringGetLength(s) != 10) {
        fprintf(stderr, "stringTest3: ERROR - Expected length 10, got %llu\n", (unsigned long long)OCStringGetLength(s));
        ok = false;
    }
    // Check a few code-points
    // index 1 → 'é' (0xE9)
    if ((unsigned char)OCStringGetCharacterAtIndex(s, 1) != 0xE9) {
        fprintf(stderr, "stringTest3: ERROR - Character at index 1 is not 0xE9 ('é')\n");
        ok = false;
    }
    // index 6 → '•' (U+2022)
    uint32_t char_at_6 = OCStringGetCharacterAtIndex(s, 6);
    if (char_at_6 != 0x2022) {  // Check full Unicode code point for '•'
        fprintf(stderr, "stringTest3: ERROR - Character at index 6 is not U+2022. Expected 0x2022, Got 0x%X\n", char_at_6);
        ok = false;
    }
    OCRelease(s);
...
static bool test_CreateAndGetCString(void) {
    ...
    if (!s) {
        fprintf(stderr, "    ERROR: createWithCString failed\n");
        return false;
    }
...
static bool test_MutableAppend(void) {
    ...
    if (!m) {
        fprintf(stderr, "    ERROR: createMutable failed\n");
        return false;
    }
...
static bool test_MutableCopy(void) {
    ...
    if (!copy) {
        fprintf(stderr, "    ERROR: createMutableCopy failed\n");
        return false;
    }
...
static bool test_TrimFunctions(void) {
    ...
    if (!ws) {
        fprintf(stderr, "    ERROR: createMutableCString failed\n");
        return false;
    }
